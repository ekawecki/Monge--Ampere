...
while (i < itermax):
    ...
    F = ( H00 * phi00 + duhx * phi00.dx(0) )*dx(mesh) \
        + ( H11 * phi11 + duhy * phi11.dx(1) )*dx(mesh) \
        + ( H01 * phi01 + duhx * phi01.dx(1) )*dx(mesh) \
        - (phi00 * n[0]) * (duhx) * ds(mesh) \
        - (phi11 * n[1]) * (duhy) * ds(mesh) \
        - (phi01 * n[1]) * (duhx) * ds(mesh) \
    ...
    class InitConditions(Expression):
        ...
        def eval(self,value,x):
            ...
            value[2] = self.H110(x)
            value[3] = self.H010(x)
            value[4] = self.duhx0(x)
            value[5] = self.duhy0(x)
            value[6] = self.C0(x)
    ...
    error2 = (pow(duhx - du0,2) + pow(duhy - du1,2))*dx(mesh)
